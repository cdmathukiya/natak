            ...
            <div class="overflow-hidden rounded-xl bg-white dark:border-gray-800 dark:bg-white/[0.01]">
                <div class="flex flex-col gap-2 px-4 py-4 border border-b-0 border-gray-100 dark:border-white/[0.05] rounded-t-xl sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div class="flex items-center gap-3">
                        <Link :href="route('team_available.create')" class="inline-flex items-center gap-2 px-4 py-3 text-sm font-medium text-white transition rounded-lg bg-brand-500 shadow-theme-xs hover:bg-brand-600">
                            <svg class="fill-current" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <div class="max-w-full overflow-x-auto">
                    <table class="min-w-full">
                        ...
                        <tbody>
                            <tr v-for="(teamAvailable, index) in teamAvailables.data" :key="index" class="border-t border-gray-100 dark:border-white/[0.05]">
                                <td class="px-4 py-3 border border-gray-100 dark:border-white/[0.05]">
                                    <p class="text-gray-700 text-theme-sm dark:text-gray-400">
                                        {{ teamAvailables.from + index }}
                                    </p>
                                </td>
                                <td class="px-4 py-3 border border-gray-100 dark:border-white/[0.05]">
                                    <span class="block font-medium text-gray-800 text-theme-sm dark:text-white/90">
                                        {{ teamAvailable.team.name }}
                                    </span>
                                </td>
                                <td class="px-4 py-3 border border-gray-100 dark:border-white/[0.05]">
                                    <span class="block font-medium text-gray-800 text-theme-sm dark:text-white/90">
                                        {{ formatDate(teamAvailable.date) }}
                                    </span>
                                </td>
                                <td class="px-4 py-[17.5px] border border-gray-100 dark:border-white/[0.05]">
                                    <div class="flex items-center w-full gap-2">
                                        <Link :href="route('team_available.edit', teamAvailable.id)" v-if="teamAvailable.is_editable" class="text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white/90 hidden">
                                        ...
                                        <Link :href="route('team_available.show', teamAvailable.id)" class="text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white/90">
                                            <i class="fa-solid fa-eye"></i>
                                        </Link>
                                        <button @click.prevent="deleteTeam(teamAvailable.id)" type="button" class="text-gray-500 hover:text-error-500 dark:text-gray-400 dark:hover:text-error-500">
                    ...
                    <Pagination :links="teamAvailables.links" />
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    watch: {
        search(value) {
            ...
            this.searchTimeOut =  setTimeout( () => {
                this.$inertia.get(route('team_availables'), { search: value }, {
                    preserveState: true,
                    replace: true,
                    onFinish: () => {
    ...
    methods: {
        ...
        deleteTeamConfirmed() {
            if (this.deleteId) {
                router.delete(route("team_available.delete", this.deleteId));
            }
        ...
        handleSort(key) {
            ...
            this.teamAvailables.data = this.sortTable(key, this.teamAvailables.data);
        },
        formatDate(dateStr) {
            if (!dateStr) return '';
