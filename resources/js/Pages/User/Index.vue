<template>
            ...
            <div class="overflow-hidden rounded-xl bg-white dark:border-gray-800 dark:bg-white/[0.01]">
                <div class="flex flex-col gap-2 px-4 py-4 border border-b-0 border-gray-100 dark:border-white/[0.05] rounded-t-xl sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div class="flex items-center gap-3">
                        <Link :href="route('user.create')" class="inline-flex items-center gap-2 px-4 py-3 text-sm font-medium text-white transition rounded-lg bg-brand-500 shadow-theme-xs hover:bg-brand-600">
                            <svg class="fill-current" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <div class="max-w-full overflow-x-auto">
                    <table class="min-w-full">
                        ...
                        <tbody>
                            <tr v-for="(user, index) in users.data" :key="index" class="border-t border-gray-100 dark:border-white/[0.05]">
                                ...
                                <td class="px-4 py-3 border border-gray-100 dark:border-gray-800">
                                    <span 
                                        :class="user.is_active
                                            ? 'bg-green-100 dark:bg-green-500/15 text-green-700 dark:text-green-500' 
                                            : 'bg-red-100 dark:bg-red-500/15 text-red-700 dark:text-red-500'" 
                                ...
                                <td class="px-4 py-[17.5px] border border-gray-100 dark:border-white/[0.05]">
                                    <div class="flex items-center w-full gap-2">
                                        <Link :href="route('user.edit', user.id)" class="text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white/90">
                                        ...
                                        <button @click.prevent="deleteUser(user.id)" type="button" class="text-gray-500 hover:text-error-500 dark:text-gray-400 dark:hover:text-error-500">
    ...
    <DeleteAlert 
        v-if="isDeleted" 
        @confirmDelete="deleteUserConfirmed"
...
export default {
    ...
    methods: {
        ...
        deleteUserConfirmed() {
            if (this.deleteId) {
                router.delete(route("user.delete", this.deleteId));
            }
        ...
        handleSort(key) {
            ...
            this.users.data = this.sortTable(key, this.users.data);
        }
    }
}
</script>